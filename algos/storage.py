...
class RolloutStorage(object):
    def __init__(self,
        ...
        if action_space.__class__.__name__ == 'Discrete':
            ...
            self.action_log_dist = torch.zeros(num_steps, num_processes, np.sum(list(action_space.nvec)))
    ...
    def _compute_truncated_value_preds(self):
        ...
        with torch.no_grad():
            ...
            for i in range(self.num_processes):
                steps = torch.nonzero(self.bad_masks[:,i,0] == 0, as_tuple=False).squeeze()
    ...
    def get_batched_action_complexity(self):
        ...
        for b in range(num_processes):
            num_traj = 0
            avg_complexity = 0
            done_steps = [0] + torch.nonzero((self.masks[:,b,0] == 0), as_tuple=False).flatten().tolist()
    ...
    def get_action_traj(self, as_string=False):
        if as_string:
            ...
            for b in range(num_processes):
                ...
                # action_str = ' '.join([str(a.item()) for a in self.actions[:,b,:]]) # Need to support multi-dim actions
                action_str = '|'.join([
                        np.array2string(a,separator=',')
                        for a in self.actions[:,b,:].cpu().numpy()])
...
class NonRecurrentRolloutStorage(object):
    def __init__(self,
                 model,
                 num_steps, num_processes, observation_space, action_space,
                 use_proper_time_limits=False,
        ...
        # Keep track of if it's continuous
    ...
    def _compute_truncated_value_preds(self):
        ...
        with torch.no_grad():
            ...
            for i in range(self.num_processes):
                ...
                value_preds = self.model.policy.value_net(self.model.policy.vf_extractor(obs))
    ...
    def feed_forward_generator(self,
        ...
        for indices in sampler:
            ...
            # we don't use the recurrent hidden states
